/**
 * Created by dmarkey on 9/22/15.s
 */
angular.module('app.controllers', [])
  ...
  .controller('SocketsCtrl', function ($scope, $http, $cookies, $ionicPopup, SocketService) {
    $http.defaults.headers.post['X-CSRFToken'] = $cookies.csrftoken;
    $scope.sockets = SocketService.query();


    $scope.onSocketRename = function (socket) {

      $scope.selected_socket = socket;

      $ionicPopup.show({
        template: '<input ng-model="selected_socket.human_name">',
        title: 'Please enter the sockets new name',
        subTitle: 'Describe what\'s connected to the socket.',
        scope: $scope,
        buttons: [
          {
            text: '<b>Save</b>',
            ...
            onTap: function (e) {
              if (!$scope.selected_socket.human_name) {
                e.preventDefault();
              } else {
                return $scope.selected_socket.human_name;
              }
            }
          }
        ]
  ...
  .controller('controllerDetailCtrl', function ($scope, myControllerService, $stateParams) {
    $scope.controller = myControllerService.get({"id": $stateParams.controllerId});
